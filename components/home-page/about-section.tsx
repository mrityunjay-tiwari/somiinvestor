    ...
    return (
        ...
        <BGGradientAboutSection>
            <div className="flex flex-col items-center w-full justify-center text-center pb-16">
                <MotionDiv initial={{y:40, opacity:0}} whileInView={{y: 0,opacity: 1}} transition={{ duration: 0.5, ease: "easeOut" }} className="mx-auto mb-2 max-w-4xl text-balance text-3xl max-md:font-semibold md:text-7xl lg:mt-16 xl:text-[3.25rem]">
                    About Us
                </MotionDiv>
                <MotionDiv initial={{y:40, opacity:0}} whileInView={{y: 0,opacity: 1}} transition={{ duration: 0.5, ease: "easeOut" }} className="mb-10">
                    <Button variant={'link'} className="">Check Somi Conveyor Beltings Ltd. Share Price on BSE <Link2 /></Button>
                </MotionDiv>
                <MotionDiv initial={{y:40, opacity:0}} whileInView={{y: 0,opacity: 1}} transition={{ duration: 0.5, ease: "easeOut" }} className="w-4/5 text-justify flex gap-12 bg-zinc-100/70 p-8 rounded-2xl">
                    <MotionDiv initial={{y:40, opacity:0}} whileInView={{y: 0,opacity: 1}} transition={{ duration: 0.5, ease: "easeOut" }} className="border shadow-xl p-5 rounded-2xl bg-white">
                        <MotionSpan initial={{x:20, opacity:0}} whileInView={{y: 0,opacity: 1}} transition={{ duration: 0.5, ease: "easeOut" }} className="font-semibold text-lg">SOMI CONVEYOR BELTINGS LTD</MotionSpan>
                        {` is a BSE Listed Public Ltd company having two manufacturing plants located at Jodhpur India, well connected by air, rail & road. The plants are equipped by 100% PLC based fully automatic & computerized machineries. Company is capable to produce New Generatio (Somiflex) Conveyor Belts upto 2000mm Width with Installed capacity of 9,00,000 meters per annum. Company manufactures the World-class Quality Conveyr Belts equivalent to Fenner, Yokohama, Dunlop, Continetal.
                    ...
                    <div className="flex flex-col">
                        <MotionDiv initial={{y:40, opacity:0}} whileInView={{y: 0,opacity: 1}} transition={{ duration: 0.5, ease: "easeOut" }} className="rounded-2xl shadow-xl">
                            <iframe width="560" height="315" src="https://www.youtube.com/embed/7lrE6iQyZtU?si=gW0h-QQ72aclA4OA&amp;controls=0" title="YouTube video player" frameBorder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen></iframe>
                        </MotionDiv>
                            <ContactSection />
                    </div>  
                </MotionDiv>
